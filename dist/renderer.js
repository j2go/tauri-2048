    ...
    switch (event.keyCode) {
        case 37: //left
            if (moveLeft()) {
                ...
                setTimeout(function(){ isgameover()}, 500);
            }
            break;
        case 38: //up
            if (moveUp()) {
        ...
        case 67:
            ...
            if (name) {
                var zs = name.split(",");
                var x = parseInt(zs[0]), y = parseInt(zs[1]);
                board[y][x] = 0;
                updateBoardView();
            }
        default:
...
document.addEventListener('touchend', function (event) {
    ...
    //x
    if (Math.abs(datax) >= Math.abs(datay) && Math.abs(datax) > 0.2 * doucmentWidth) {
        //right
        if (datax > 0) {
            if (moveRight()) {
    ...
    else if (Math.abs(datay) > 0.2 * doucmentWidth)//y
    {
        //up
        if (datay < 0) {
            if (moveUp()) {
